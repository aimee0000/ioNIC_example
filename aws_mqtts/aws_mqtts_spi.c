...
char spi_tx_buf[BUF_SIZE] = {0,};
char spi_rx_buf[BUF_SIZE] = {0,};

// ======= AWS MQTTS Information =======
const char* SERVER_IP   = "a2heyxmgdlstqu-ats.iot.ap-southeast-2.amazonaws.com";    // Remote Host: AWS endpoint
const char* SERVER_PORT = "8883";                                                   // Remote Port 
const char* MQTT_CLIENT_ID = "my_thing";                                            // AWS Things name
const char* PUB_TOPIC = "$aws/things/my_thing/shadow/update";                       // AWS Publish Topic
const char* SUB_TOPIC = "$aws/things/my_thing/shadow/update/accepted";              // AWS Subsribe Topic
...
int main() {
    ...
    printf("=== W55RP20-S2E AWS MQTTS Demo (SPI mode) ===\n");
    ...
    printf("\n--- AWS MQTTS Client Running ---\n");
    snprintf(spi_tx_buf, sizeof(spi_tx_buf), "Hello ioNIC AWS MQTTS Test\r\n");

    while (true) {
        if (spi_rx_pending) {
            memset(spi_rx_buf, 0x0, sizeof(spi_rx_buf));
            int len = spi_read(spi_rx_buf, sizeof(spi_rx_buf));

            if (len > 0) {
                printf("%s\r\n", spi_rx_buf);
                bool ok = spi_write(spi_tx_buf, strlen(spi_tx_buf));
                if(ok == false)
