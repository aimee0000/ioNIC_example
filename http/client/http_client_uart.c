...
// ================== HTTP Helpers ==================
bool http_get_send(const char* host, const char* path)
{
    ...
    if (len <= 0 || len >= (int)sizeof(uart_tx_buf)) {
        printf("HTTP request build failed (len=%d)\n", len);
        return false;
    }
...
void http_rx_print_loop(uint32_t idle_timeout_ms)
{
    uint32_t rx_idx = 0;
...
int main() {
    ...
    printf("=== W55RP20-S2E HTTP Client Demo (UART mode) ===\n");
