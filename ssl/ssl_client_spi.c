...
const char* SERVER_PORT = "443";             // PC(Server) Port 
...
int main() {
    ...
    printf("=== W55RP20-S2E SSL Client Demo (SPI mode) ===\n");
    ...
    printf("\n--- Config W55RP20 with AT command(SPI) ---\n");
    {
        ...
        sleep_ms(3000); 
        at_set("OP", "4");                      // Set W55RP20 SSL client mode
        ...
        at_set("SO", "5000");                   // Set SSL receive timeout
    ...
    sleep_ms(5000);
    printf("\n--- SSL Client Running ---\n");
    printf("PC should run SSL server (example): openssl s_server -accept %s -cert server.crt -key server.key\n", SERVER_PORT);
    
    snprintf(spi_tx_buf, sizeof(spi_tx_buf), "Hello ioNIC ssl Test\r\n");
